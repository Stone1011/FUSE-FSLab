...
//    }
    ...
    for(int i = 0; i < blockNum; i++)
    {
        ...
        else
        {
            ...
            readDataBlock(current.indirPointer[0], buffer2);
            int * indir = (int *)buffer2;
            readDataBlock(indir[i - 14], buffer);
            struct DirectoryDataBlock *dir = (struct DirectoryDataBlock *) buffer;
            for (int j = 0; j < dir->childNum; j++)
            {
                struct Directory now = dir->children[j];
...
//            return -ENOSPC;
            ...
            writeDataBlock(newBlock, buffer2);
            current->indirPointer[0] = newBlock;
        }
        blockNum ++;
        current->blocks = blockNum;
        int newBlock = getEmptyDataBlock();
        ...
        else
        {
            readDataBlock(current->indirPointer[0], buffer2);
            int * indir = (int *)buffer2;
            indir[current->blocks - 1 - 14] = newBlock;
            writeDataBlock(current->indirPointer[0], buffer2);
        }
        dir->childNum = 1;
    ...
    else
    {
        ...
        if(current->blocks <= 14)
            writeDataBlock(current->dirPointer[current->blocks - 1], buffer);
        else
        {
            readDataBlock(current->indirPointer[0], buffer2);
            int * indir = (int *)buffer2;
            writeDataBlock(indir[current->blocks - 1 - 14], buffer);
        }
    }
...
int fs_rename (const char *oldpath, const char *newpath)
{
    ...
    char tempOld[2560], nameOld[32];
    char tempNew[2560], nameNew[32];
...
//                }
                ...
                if(sizes > 0)
                    writeInodeData(&inode, buf, sizes);
                free(tempBlock);
            }
        }
        else
